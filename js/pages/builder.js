...
var KTLayoutBuilder = function() {
	...
	var exporter = {
		...
		startLoad: function(options) {
			...
			find('.btn').
			attr('disabled', true);
			infoAlert(options.title, options.message);
		},
		doneLoad: function() {
			$('#builder_export').
				...
				var timer = setInterval(function() {
					$.ajax('index.php', {
						...
						$('<iframe/>').attr({
							...
							successAlert('Export HTML Version Layout', 'HTML version exported.');
							exporter.doneLoad();
							// stop the timer
							clearInterval(timer);
						}).appendTo('body');
				...
				var timer = setInterval(function() {
					$.ajax('index.php', {
						...
						$('<iframe/>').attr({
							...
							successAlert('Export Default Version', 'Default HTML version exported with current configured layout.');
							exporter.doneLoad();
							// stop the timer
							clearInterval(timer);
						}).appendTo('body');
	...
	var preview = function() {
		$('[name="builder_submit"]').click(function(e) {
			...
			$.ajax('index.php?demo=' + $(_self).data('demo'), {
				...
				successAlert('Preview updated', 'Preview has been updated with current configured layout.');
			}).always(function() {
				setTimeout(function() {
					location.reload();
				}, 600);
